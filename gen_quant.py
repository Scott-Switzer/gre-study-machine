...
# ---------------- ARITHMETIC ----------------
for _ in range(45):
    k=random.choice(["pct","disc","ratio","avg","exp","root","div","prime","seq"])
    if k=="pct":
        ...
        add(mc(f"A price is reduced by {p}% then by a further {q}%. What is the total percent discount from the original?",
               disc,[p+q,100-final,p-q,round(p*q/100,1)],"Arithmetic: Percent","medium",
        ...
        g=math.gcd(r1,r2) or 1
        s=(r1+r2)//g if g else r1+r2
        part=round(tot*r1/(r1+r2))
        other=round(tot*r2/(r1+r2))
        ...
        def isp(x):
            ...
            return True
        # find a prime near cand
        p=cand
        while not isp(p): p+=1
        add(mc(f"What is the smallest prime number greater than {cand-1}?", p,[p+random.choice([2,4,6]),p-1,p+1 if isp(p+1) else p+2, p+10],
        ...
        add(mc(f"An arithmetic sequence starts {terms[0]}, {terms[1]}, {terms[2]}, ... (common difference {d}). What is the {n}th term?",
               an,[an+d,an-d,a0+d*n,an+1],"Arithmetic: Sequences","easy",
               f"a_n = a_1 + (n-1)d = {a0} + ({n}-1)({d}) = {an}."))

# ---------------- ALGEBRA ----------------
for _ in range(45):
    k=random.choice(["lin","two","quad","ineq","func","wp"])
    if k=="lin":
        ...
        add(mc(f"Solve the system:\n{a1}x + {b1}y = {c1}\n{a2}x + {b2}y = {c2}\nWhat is x + y?", x+y,
               ...
               f"The unique solution is x={x}, y={y} (verify both equations). So x+y={x+y}."))
    elif k=="quad":
        r1=random.randint(1,7); r2=random.randint(1,7)
        if random.random()<0.5:
            b=-(r1+r2); c=r1*r2
            disc=b*b-4*c; rt=(-b+math.isqrt(disc))//2 if disc>=0 else r1
        ...
        f=lambda t:a*t+b
        add(mc(f"f(x) = {a}x + {b}. What is f({x})?", f(x),[f(x)+1,f(x)-1,a*x,b],"Algebra: Functions","easy",
               f"f({x}) = {a}({x}) + {b} = {f(x)}."))
    else: # wp distance/rate
        r=random.randint(20,80); t=random.randint(2,6)
        d=r*t
        add(mc(f"A car travels at {r} mph for {t} hours. How many miles does it travel?", d,
               [r+t, r*t*random.choice([2,3]), abs(r-t), d+10],"Algebra: Word Problem","easy",
               f"Distance = rate x time = {r} x {t} = {d} miles."))

# ---------------- GEOMETRY ----------------
for _ in range(40):
    k=random.choice(["rect","tri","circ","pyth","vol","ang","coord"])
    if k=="rect":
        ...
        add(mc(f"A triangle has base {b} and height {h}. What is its area?", b*h//2,
               [b*h, b*h*2, (b+h)//2, b*h//2+1],"Geometry: Area","easy",f"Area = 1/2 x base x height = 1/2 x {b} x {h} = {b*h//2}."))
    elif k=="circ":
        ...
        add(mc(f"A cube has side length {s}. What is its volume?", s**3,[6*s*s,s*s,s*4,s**3//2],
               "Geometry: Volume","easy",f"Volume = s^3 = {s}^3 = {s**3}."))
    elif k=="ang":
        ...
        add(mc(f"What is the sum of the interior angles of a regular {n}-gon?", (n-2)*180,
               ...
               f"Sum = (n-2) x 180 = ({n}-2) x 180 = {(n-2)*180} degrees."))
    else: # coord
...
# ---------------- DATA (stats/prob/counting) ----------------
for _ in range(40):
    k=random.choice(["mean","median","prob","comb","venn"])
    if k=="mean":
        ...
        med=arr[n//2]
        add(mc(f"What is the median of: {', '.join(map(str,arr))}?", med,[sum(arr)//n,arr[0],arr[-1],med+1],
               "Data: Statistics","easy",f"Ordered middle value of {n} numbers is the 3rd = {med}."))
    elif k=="prob":
        red=random.randint(2,6); blue=random.randint(2,6); tot=red+blue
        p=round(red/tot,3)
        add(mc(f"A bag has {red} red and {blue} blue marbles. What is P(red) when one is drawn?", p,
               [round(blue/tot,3),round(1/2,3),round(red/(tot-1),3),round((red+1)/tot,3)],"Data: Probability","medium",
               f"P(red) = red/total = {red}/{tot} = {p}."))
    elif k=="comb":
        n=random.randint(4,8); r=2
        val=math.comb(n,r)
        add(mc(f"How many ways to choose {r} items from {n}? (order doesn't matter)", val,
               [n**r, math.perm(n,r), n+r, val+random.choice([1,-1])],"Data: Counting","medium",
               f"C({n},{r}) = {n}! / ({r}!({n}-{r})!) = {val}."))
    else: # venn / overlap
        ...
        add(mc(f"In a group, {A} like tea, {B} like coffee, and {both} like both. How many like at least one?",
               tot,[A+B,A+B+both,abs(A-B),both],"Data: Sets","medium",
               f"At least one = A + B - both = {A} + {B} - {both} = {tot}."))

# ---------------- QUANTITATIVE COMPARISON ----------------
for _ in range(60):
    k=random.choice(["qc-arith","qc-alg","qc-geo","qc-var"])
    if k=="qc-arith":
        ...
        add(qc(f"x = {a}, y = {b}\nQuantity A: x + y\nQuantity B: 2y",
               ...
               "easy" if rel!="D" else "medium",f"x+y = {a+b}, 2y = {2*b}. Compare {a+b} vs {2*b}."))
    elif k=="qc-alg":
        x=random.randint(2,9)
        add(qc(f"x > 1\nQuantity A: x^2\nQuantity B: x",
               "A","QC: Algebra","easy",f"For x>1, x^2 > x (e.g., x={x}: {x*x} > {x})."))
    elif k=="qc-geo":
        ...
        add(qc(f"A circle has radius {r}.\nQuantity A: circumference (2*pi*r)\nQuantity B: area (pi*r^2)",
               ...
               f"Compare 2r and r^2 (pi cancels): 2r vs r^2 -> for r={r}, " + ("2r > r^2" if 2>r else "r^2 > 2r") + "."))
    else: # variable -> D
        add(qc(f"x is an integer.\nQuantity A: x^2\nQuantity B: x",
               "D","QC: Variable","medium",
               "If x=2, A=4>B=2; if x=0, A=0=B; if x=-1, A=1>B=-1; if x=1/2 (not integer but) A<B. Since x can be 0 or 1 (A=B) or >1 (A>B), the relationship is not fixed -> D."))
...
print("By topic count sample:", len(set(q["topic"] for q in Q)))
